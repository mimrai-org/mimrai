...
import { session, users } from "@mimir/db/schema";
import {
	generateAuthUrl,
	oauth2Client,
} from "@mimir/integration/google-calendar";
...
app.get("oauth", ...protectedMiddleware, async (c) => {
	...
	await db
		.update(session)
		.set({ metadata: { googleCalendarState: state } })
...
app.get("oauth/callback", ...protectedMiddleware, async (c) => {
	...
	const [dbSession] = await db
		...
		.where(eq(session.id, cSession.id))
		.limit(1);
	const dbState = dbSession?.metadata?.googleCalendarState;
